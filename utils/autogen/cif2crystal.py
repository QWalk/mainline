...
# this is relative to the run directorys. You can also put an absolute position
...
def modify_basis(symbol,xml_name,cutoff=0.2,params=[0.2,2,3],initial_charges={}):
    ...
    return ["%i %i"%(mg.Element(symbol).number+200,ncontract)] +\
...
class Cif2Crystal:
  ...
  def run(self,job_record):
    ...
    outlines += ["XLGRID","END","SCFDIR","BIPOSIZE","100000000"]
    outlines += ["FMIXING",str(job_record['dft']['fmixing'])]
    outlines += ["BROYDEN",
        "%g %i %i"%(job_record['dft']['broyden'][0],
                      job_record['dft']['broyden'][1],
                      job_record['dft']['broyden'][2])]
    outlines += ["TOLINTEG","12 12 12 12 20"]
    if job_record['dft']['spin_polarized']:
