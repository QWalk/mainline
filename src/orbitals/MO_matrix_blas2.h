...
#ifndef MO_MATRIX_BLAS2_H_INCLUDED
#define MO_MATRIX_BLAS2_H_INCLUDED
...
template <class T> class MO_matrix_blas2: public Templated_MO_matrix <T> {
protected:
  void init();
...
template <class T> void MO_matrix_blas2<T>::init() {
...
template <class T> void MO_matrix_blas2<T>::buildLists(Array1 < Array1 <int> > & occupations)
{
...
template <class T> int MO_matrix_blas2<T>::showinfo(ostream & os)
{
  os << "Blas MO " << endl;
...
template <class T> int MO_matrix_blas2<T>::writeinput(string & indent, ostream & os)
{
  os << indent << "BLAS2_MO" << endl;
...
inline void product_kernel2(int n, doublevar & a, doublevar * x, doublevar * y) { 
#ifdef USE_BLAS
...
inline void product_kernel2(int n, dcomplex & a, dcomplex * x, dcomplex * y) { 
#ifdef USE_BLAS
...
template <class T> struct MOBLAS2_CalcObjVal { 
  T * moplace;
  T sval;
};

template <class T> struct MOBLAS2_CalcObjLap { 
  T * moplace;
  T sval[5];
};
...
template <class T> void MO_matrix_blas2<T>::updateVal(Sample_point * sample,
  ...
  static Array1 <MOBLAS2_CalcObjVal<T> > calcobjs(totbasis);
  int ncalcobj=0;
  ...
  for(int i=0; i< ncalcobj; i++) { 
    product_kernel2(nmo_list,calcobjs(i).sval,calcobjs(i).moplace,newvals_T.v);
...
template <class T> void MO_matrix_blas2<T>::updateLap(
  Sample_point * sample, 
  int e, 
  int listnum,
  Array2 <T> & newvals) {
  ...
  static Array1 <MOBLAS2_CalcObjLap<T> > calcobjs(totbasis);
  int ncalcobj=0;
  ...
  for(int i=0; i< ncalcobj; i++) { 
    for(int j=0; j< 5; j++) { 
      product_kernel2(nmo_list,calcobjs(i).sval[j],calcobjs(i).moplace,
...
#endif // MO_MATRIX_BLAS2_H_INCLUDED
